 ...
 *  
 */
public class PlanTrackImportAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		List<List> lists = new ArrayList();
    	  ...
    	  for (int i = 1; i <= rowcount; i++) {
    		  if(ex!=null&&ex.equals("xls")){
    			  listuser =  ExcelReadService.readRow1(0, i);
    			}else if(ex!=null&&ex.equals("xlsx")){
    		  ...
    		  for (int i = 1; i <= rowcount; i++) {
        		  ...
        		  for (int j = 0; j < lists.get(i-1).size(); j++) {
    	    			  if(lists.get(i-1).get(j)!=null){
    	    				  ...
    	    				  case 3: p.setProcurementWayNum(lists.get(i-1).get(j).toString());break;
    	    				  case 4: p.setbNumber(lists.get(i-1).get(j).toString());break;
    	    				  }
    	    			  }else{
    	    				  continue;
    	    			  }
        		    }
        		  pservice.modifyPlanTrack(p);
        	  }
    		  if(f.exists()){
    			  f.delete();
    		  }
    	  }
		return "importplantrackfiles_success";
	}
	/**
	 * 
	 * @param list
...
//			if(i!=1 && i!=2){
				if(list.get(i)==null){
					l.put(i, "为空值");
					continue;
				}else if(i==2 && list.get(i-1)!=null){
					List sl = pservice.getPlanListByOldPlanNum(list.get(i-1).trim(), list.get(i).trim());
					if(sl.size() < 1){
						l.put(i-1, "没有此购物车号");
						l.put(i, "没有此行号");
						continue;
...
//			}
	...
	public  boolean isNumeric1(String str){
		try {
			...
			if(strd==1 || strd==2 || strd==3 || strd==4){
				return true;
			}
