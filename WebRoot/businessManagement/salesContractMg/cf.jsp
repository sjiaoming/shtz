...
function getContractNum(feil,num){
	...
	var ctemp=0;
	var temp = document.getElementsByName("contractNum");
	if(temp.length>0){
...
function ok(){
		...
		if(access){
			if(confirm("您确定要提交吗?")){
				this.form1.action = "cfPlan.action";
				this.form1.submit();
				//top.Dialog.close();
				return;
			}
...
<form action="?" id="form1" name="form1" >
	...
	<table id="tab1" class="tableStyle" >
		<tr>
			<td colspan="2"><b>总数量：${param.num }</b></td>
		</tr>
		<tr >
			<td><input type="text" name="contractNum"  onclick="getContractNum(this,${param.num })"/></td>
