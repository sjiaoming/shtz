...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type='text/javascript'
		src='<%=path%>/dwr/interface/planManager.js'></script>
	...
	<body onload="initPage()">
		<div class="box2" panelTitle="功能面板 " roller="false">
			<form id="form1" name="form1" action="#" method="post">
				...
				<input type="hidden" id="contractMsg" name="contractMsg"
					value="${contractMsg }">
				<input type="hidden" id="all_select" name="all_select"
					value="${all }">
				<input type="hidden" name="ccflag" id="ccflag" value="${ccflag }" />
				<input type="hidden" name="add" id="add" value="${add }" />
				<input type="hidden" name="myOffset" id="myOffset"
					value="${myOffset }" />
				...
				<table>
					<tr>
								...
								<c:if test="${!empty sessionOrgs}">
									<c:forEach items="${sessionOrgs }" var="organization">
										<option value="${organization.id}"
											<c:if test="${organization.id == params.id_org}">selected="selected"</c:if>>
											${organization.name }
										</option>
									</c:forEach>
								</c:if>
						...
						<td>
							<input type="text" name="oldPlanNum"
								value="${params.oldPlanNum }" />
						</td>
						<td>
							搜备注：
						</td>
						<td>
							<input type="text" name="remark" value="${params.remark }" />
						</td>
						<td>方案号:</td>
						...
						<td>
							<input type="text" name="bNumber" value="${params.bNumber }" />
						</td>
						<td>
							操作人：
						</td>
						...
						<c:if test="${my:method(login.id,'salescontractMg',1)}">
							<td>
								<button onclick='showProgressBar();search(0);'>
					...
					<c:if test="${my:method(login.id,'salescontractMg',0)}">
						<tr>
							<td colspan="7">
								...
								<button type="button"
									onclick='top.Dialog.confirm("您确定要重新选择？",function(){resetmethod()});'>
									<span class="icon_reload">重新选择</span>
								</button>
								<!-- <input type="button" value="生成销售合同" onclick='gettgvalue()'/> -->
								<button type="button"
									onclick='top.Dialog.confirm("您确定要生成销售合同？",function(){openWin()});'>
									<span class="icon_ok">生成销售合同</span>
								</button>
							</td>
							<td colspan="4">
								<font color="red"><b><div id="selectCount"></div>
								</b>
								</font>
							</td>
						</tr>
					...
					<tr>
						<td colspan="11">
							<b> 总条数：<fmt:formatNumber value="${pm.total1}"
									pattern="#0.00" />&nbsp;&nbsp;&nbsp;&nbsp; 总合同签订数量：<fmt:formatNumber
									value="${pm.double1 }" pattern="#0.00" />&nbsp;&nbsp;&nbsp;&nbsp;
								总预采购金额：<fmt:formatNumber value="${pm.double2 }" pattern="#0.00" />&nbsp;&nbsp;&nbsp;&nbsp;
							</b>
						</td>
			...
			<tbody>
				<c:if test="${!empty pm.list}">
					<c:forEach items="${pm.list }" var="plan" varStatus="index">
						<tr>
							<td>
								<input id="tg_${plan.id }" name="checkbox" type="checkbox"
									value="${plan.id }" onclick='getSelectedCount()' />
							</td>
							<td>
								${index.count }
							</td>
							<td>
								${plan.reportComp }
								<input type="hidden" id="reportCompId_${plan.id }"
									name="reportCompId_${plan.id }" value="${plan.reportCompId }" />
							</td>
							<td>
								${plan.oldPlanNum }
							</td>
							...
							<td>
								<fmt:formatNumber value="${plan.procurementMoney }"
									type="currency" />
							</td>
							<td>
								${plan.procurementRemark }
							</td>
							<td>
								${plan.remark }
							</td>
							<td>
								${plan.procurementWayNum}
							</td>
							<td>
								${plan.bNumber }
							</td>
							...
							<td>
								<span class="img_edit hand" title="拆分计划"
									onclick='cf(${plan.id },${plan.num });'></span>
							</td>
						</tr>
					</c:forEach>
				</c:if>
		...
		<div style="height: 35px;">
			<pg:pager items="${pm.total }" url="salesContractSearch.action"
				export="currentPageNumber=pageNumber" maxPageItems="50">

				<div class="float_right padding5 paging">
					<div class="float_left padding_top4">
						<c:choose>
							<c:when test="${ currentPageNumber == 1}">
								<span class="paging_disabled"><a href="javascript:;">首页</a>
								</span>
							</c:when>
							<c:otherwise>
								<span><pg:first>
										<a href="#" onclick="getCheckBoxInformationnn('${pageUrl }')">首页</a>
									</pg:first>
								</span>
							</c:otherwise>
						</c:choose>
						<c:choose>
							<c:when test="${ currentPageNumber > 1}">
								<span><pg:prev>
										<a href="#" onclick="getCheckBoxInformationnn('${pageUrl }')">上一页</a>
									</pg:prev>
								</span>
							</c:when>
							<c:otherwise>
								<span class="paging_disabled"><a href="javascript:;">上一页</a>
								</span>
							</c:otherwise>
						</c:choose>
						...
						<pg:pages>
							<c:choose>
								<c:when test="${ currentPageNumber eq pageNumber}">
									<span class="paging_current"><a href="javascript:;">${pageNumber}</a>
									</span>
								</c:when>
								<c:otherwise>
									<span><a href="#"
										onclick="getCheckBoxInformationnn('${pageUrl }')">${pageNumber
											}</a>
									</span>
								</c:otherwise>
							</c:choose>
						...
						<span><pg:next>
								<a href="#" onclick="getCheckBoxInformationnn('${pageUrl }')">下页</a>
							</pg:next>
						</span>
						<span><pg:last>
								<a href="#" onclick="getCheckBoxInformationnn('${pageUrl }')">尾页</a>
							</pg:last>
						</span>
						<div class="clear"></div>
					</div>
